...
class Media extends React.Component {
    ...
    render() {
        ...
        const redirectComponent = <Redirect to={{pathname: `/${this.state.clickedUserType}Profile`, state: { clickedUser: this.state.clickedUser}}}/> ;
    ...
    like = (i, likeNum, e) => {
        ...
        if (this.state.liked[i]) {
            ...
            const tempLiked = this.state.liked
            tempLiked[i] = false;
            this.setState( {liked: tempLiked} );
        } else {
            this.props.handleLike(i, 1);
